...
| 15 | Research outputs | 🟡 | Generated briefings under `/reports/`: an **annual briefing** per conference-year (stats, sessions by day, themes, organizations, recordings, quotes) and a **cross-year theme briefing** per topic (sessions per year, organizations and people active, a standards/references reading list, and quotes), plus an index. All aggregated from the datasets and linked back to the records. Per-speaker/organization reports and a standalone repository/standards index are still to come. |
